...
(function () {
  ...
  GEN.grid = function (W, H, v, ac, opts) {
    var dark = v === 'dark', m = '', col = '#1283DD';
    var yoff = (opts && opts.gridYOffset) || 0;
    ...
    for (var gx = major; gx < W; gx += major) {
      for (var gy = major; gy < H; gy += major) {
        ...
        m += '<line x1="' + (gx - L) + '" y1="' + fy + '" x2="' + (gx + L) + '" y2="' + fy + '" stroke="' + c + '" stroke-width="' + (hot ? 1.3 : 0.9) + '" stroke-opacity="' + o + '"/>';
        m += '<line x1="' + gx + '" y1="' + (fy - L) + '" x2="' + gx + '" y2="' + (fy + L) + '" stroke="' + c + '" stroke-width="' + (hot ? 1.3 : 0.9) + '" stroke-opacity="' + o + '"/>';
        if (hot) m += '<circle cx="' + gx + '" cy="' + fy + '" r="3.2" fill="none" stroke="' + ac + '" stroke-width="1.1" stroke-opacity="0.85"/>';
      }
    }
    return { bg: dark ? '#030a17' : '#f5f7fa', inner: m };
  };
  ...
  function render(el, opts) {
    ...
    var out = gen(W, H, variant, accent, opts);
